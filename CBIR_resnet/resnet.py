...
class ResNetFeat(object):
  ...
  def make_samples(self, db, verbose=True):
    ...
    return samples
  def getFeatureQuery(self,Img,verbose=True):
    sample_cache = '{}-{}'.format(RES_model, pick_layer)
  
    try:
      samples = cPickle.load(open(os.path.join(cache_dir2, sample_cache), "rb", True))
      for sample in samples:
        sample['hist'] /= np.sum(sample['hist'])  # normalize
      if verbose:
        print("Using cache..., config=%s, distance=%s, depth=%s" % (sample_cache, d_type, depth))
      ...
      try:
        ...
        d_hist = d_hist.data.cpu().numpy().flatten()
        d_hist /= np.sum(d_hist)  # normalize
        samples.extend(d_hist)
      except:
        pass
    cPickle.dump(samples, open(os.path.join(cache_dir2, sample_cache), "wb", True))
    return samples
if __name__ == "__main__":
  ...
  res = ResNetFeat()
  #---
  features = res.make_samples(db)
  filename = "index.csv"
  arrmain = []
  for k in range(len(features)): 
    arr = np.append(features[k]['img'],features[k]['hist'])
    arrmain.append(arr)
  print(arrmain)
  with open(filename, 'w') as csvfile:  
    #creating a csv writer object  
    csvwriter = csv.writer(csvfile)  
    for k in range(len(arrmain)):    
    # writing the data rows  
      csvwriter.writerow(arrmain[k])  
  import csv
  with open('index.csv') as input, open('output.csv', 'w') as output:
